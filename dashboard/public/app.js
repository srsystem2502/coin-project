  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      const message = url === '/api/metadata'
        ? { saved: true, githubUpdated: result.githubUpdated, note: result.githubUpdated ? 'metadata.json GitHub updated' : 'No GitHub token. On-chain updated only; metadata-preview.json saved locally.' }
        : result;
      log(message);
